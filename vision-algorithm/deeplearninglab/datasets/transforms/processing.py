...
from ...utils import is_seq_of, is_list_of, is_tuple_of
...
class Normalize(BaseTransform):
    ...
    def transform(self, results: dict) -> dict:
        ...
        results['img'] = impots.imnormalize(results['img'], self.mean, self.std,
                                          self.to_rgb)
...
class Resize(BaseTransform):
    ...
    def _resize_seg(self, results: dict) -> None:
        ...
        if results.get('gt_seg_map', None) is not None:
            if self.keep_ratio:
                gt_seg = impots.imrescale(
                    results['gt_seg_map'],
                    results['scale'],
                    interpolation='nearest',
                    backend=self.backend)
            else:
                gt_seg = impots.imresize(
                    results['gt_seg_map'],
                    results['scale'],
                    interpolation='nearest',
                    backend=self.backend)
...
class Pad(BaseTransform):
    ...
    def _pad_seg(self, results: dict) -> None:
        ...
        if results.get('gt_seg_map', None) is not None:
            ...
            results['gt_seg_map'] = impots.impad(
                results['gt_seg_map'],
...
class CenterCrop(BaseTransform):
    ...
    def _crop_img(self, results: dict, bboxes: np.ndarray) -> None:
        ...
        """
        if results.get('img', None) is not None:
            img = impots.imcrop(results['img'], bboxes=bboxes)
    ...
    def _crop_seg_map(self, results: dict, bboxes: np.ndarray) -> None:
        ...
        """
        if results.get('gt_seg_map', None) is not None:
            img = impots.imcrop(results['gt_seg_map'], bboxes=bboxes)
...
class RandomFlip(BaseTransform):
    ...
    def _flip_seg_map(self, seg_map: dict, direction: str) -> np.ndarray:
        ...
        """
        seg_map = impots.imflip(seg_map, direction=direction)
