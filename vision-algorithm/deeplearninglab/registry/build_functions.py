...
def build_from_cfg(
    ...
    if not (isinstance(default_args, (dict, ConfigDict, Config)) or default_args is None):
        raise TypeError('default_args should be a dict, ConfigDict, Config or None, 'f'but got {type(default_args)}')
    ...
    with registry.switch_scope_and_registry(scope) as registry:
        ...
        if isinstance(obj_type, str):
            ...
            if obj_cls is None:
                raise KeyError(f'{obj_type} is not in the {registry.scope}::{registry.name} registry. '  # noqa: E501
                    f'Please check whether the value of `{obj_type}` is ''correct or it was registered as expected.')
            ...
            if isinstance(scheduler_type, str):
                ...
                if scheduler_cls is None:
                    raise KeyError(
                        ...
                        f'`{scheduler_type}` is correct or it was registered as expected.'
                    )
